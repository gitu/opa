...
func (t *testServer) handle(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(r.URL.Path, "/v2/org/repo/") {
		// load the testdata hashes
		if t.testdataHashes == nil {
			// read the testdata hashes
			...
			for _, file := range files {
				...
				hash, length, err := getFileSHA("testdata/" + file.Name())
				if err != nil {
	...
	if strings.HasPrefix(r.URL.Path, "/v2/org/repo/manifests/") {
		sha, size, err := getFileSHA("testdata/" + strings.TrimPrefix(r.URL.Path, "/v2/org/repo/manifests/") + ".manifest")
		if err != nil {
			w.WriteHeader(404)
			return
		}
...
func getFileSHA(filePath string) ([]byte, int64, error) {
